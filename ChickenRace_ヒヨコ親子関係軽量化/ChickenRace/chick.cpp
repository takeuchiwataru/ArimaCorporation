...
LPD3DXMESH				CChick::m_pMesh[MAX_CHICK_PARTS] = {};			//メッシュ情報の初期化
LPD3DXBUFFER			CChick::m_pBuffMat[MAX_CHICK_PARTS] = {};		//マテリアルの情報の初期化
DWORD					CChick::m_nNumMat[MAX_CHICK_PARTS] = {};		//マテリアルの情報数の初期化
...
HRESULT CChick::Init(void)
{
	...
	for (int nCountIndex = 0; nCountIndex < MAX_CHICK_PARTS; nCountIndex++)
	{// 減速鶏のポインタを初期化
	 //NULLチェック
		if (m_apModel[nCountIndex] == NULL)
		{//動的確保
			m_apModel[nCountIndex] = new CModel;
			//NULLチェック
			if (m_apModel[nCountIndex] != NULL)
			{
				//モデルの生成
				//m_apModel[nCountIndex]->BindModel(m_pMesh[nCountIndex], m_pBuffMat[nCountIndex], m_nNumMat[nCountIndex]);
				m_apModel[nCountIndex]->Init();
			}
		}
...
{
	for (int nCount = 0; nCount < MAX_CHICK_PARTS; nCount++)
	{
		// メッシュの開放
...
bool CChick::CollisionChick(D3DXVECTOR3 * pPos, D3DXVECTOR3 * pPosOld)
{
	...
	if (m_state == STATE_BULLET)
	{
		...
		D3DXVECTOR3 ModelMax = m_pos + VtxMax; // 位置込みの最大値
		D3DXVECTOR3 ModelMin = m_pos + VtxMin; // 位置込みの最小値
