...
public://誰でも扱える
	...
	static D3DXVECTOR3 GetVtxMax(void) { return m_VtxMax; }
	static D3DXVECTOR3 GetVtxMin(void) { return m_VtxMin; }
...
private://派生クラスでも使える
	...
	static D3DMATERIAL9			*m_pMeshMaterials[MAX_MODEL];
	static LPDIRECT3DTEXTURE9	*m_pShaderMeshTextures[MAX_MODEL];			//シェーダー用
	static LPDIRECT3DTEXTURE9	*m_pMeshTextures[MAX_MODEL_TEXTURE];		// シェーダー用
	static LPD3DXBUFFER			m_pBuffMat[MAX_MODEL];						//マテリアルの情報へのポインタ
	static LPD3DXMESH			m_pMesh[MAX_MODEL];							//メッシュ情報へのポインタ
	static DWORD				m_nNumMat[MAX_MODEL];						//マテリアルの情報数
	static D3DXVECTOR3			m_VtxMin, m_VtxMax;							//モデルの最小値・最大値
