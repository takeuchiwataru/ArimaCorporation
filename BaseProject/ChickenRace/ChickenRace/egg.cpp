...
void CEgg::Item(void)
{
	if (m_eggState == EGGSTATE_BULLET)
	{
		...
		case EGGTYPE_SPEED:
			...
			if (pPlayer[m_nNumPlayer]->GetPlayerState() == CPlayer::PLAYERSTATE_NORMAL || pPlayer[m_nNumPlayer]->GetPlayerState() == CPlayer::PLAYERSTATE_DAMAGE || pPlayer[m_nNumPlayer]->GetPlayerState() == CPlayer::PLAYERSTATE_SPEEDDOWN_S) // ながやま修正1/16
			{// 加速状態じゃなくなったら削除
				Uninit();
			}
			break;
