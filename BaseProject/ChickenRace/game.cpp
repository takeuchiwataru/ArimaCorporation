...
HRESULT CGame::Init()
{
	...
	m_gameMode = GAMEMODE_COURSE_VIEW;	// ゲームモード
...
void CGame::Ranking(void)
{
	...
	for (int nCount = 0; nCount < m_nMaxPlayer; nCount++)
	{// プレイヤーカウント
		// ランキング
		...
		CDebugProc::Print("%d : %d : %s\n", nCount, m_nRanking[nCount], (m_bGoul[nCount] ? "ゴール" : "まだー"));
	}
