...
bool	CCOL_MESH_MANAGER::Collision(D3DXVECTOR3& pos, D3DXVECTOR3 &posold, D3DXVECTOR3 &move, float &fLength, D3DXVECTOR3 &FNor, bool &bJump, int &nMap)
{
	...
	for (int nCnt = 0; nCnt < CCOL_MESH_MANAGER::TYPE_MAX; nCnt++)
	{
		...
		if (FNumCollision(nCnt, mypos, pos, posold, move, FNor, bJump)) { bLand = true; }
	}
	return bLand;
}
//==================================================================================================//
...
{
	if (nNumber >= TYPE_MAX) { return false; }
	...
	if (m_pColMesh[nNumber] != NULL)
	{
		for (int nCount = 0; nCount < m_pColMesh[nNumber]->m_nNumField; nCount++)
		{
			...
			if (pMesh->MeshField(mypos, pos, posold, move, FNor, bJump)) { bLand = true; }
		}
